...
final class AudioPanel extends JPanel {
    ...
    AudioPanel() {
        ...
        SuggestionPopup.install(bus, this::availableAudioBuses);
        SuggestionPopup.install(triggerSource, () -> java.util.Arrays.asList("physics", "input", "dom"));
        SuggestionPopup.install(triggerType, () -> java.util.Arrays.asList("collisionStart", "collisionEnd", "down", "up", "click", "mouseover", "mouseout"));
        SuggestionPopup.install(triggerEventId, this::availableEventIds);
        SuggestionPopup.install(triggerAction, () -> java.util.Arrays.asList("play", "pause", "stop", "toggle", "mute", "unmute"));
    ...
    private java.util.List<String> availableAudioBuses() {
        java.util.LinkedHashSet<String> out = new java.util.LinkedHashSet<String>();
        out.add("main");
        collectAttributeValues(currentDocument, ATTR_BUS, out);
        return new java.util.ArrayList<String>(out);
    }

    private java.util.List<String> availableEventIds() {
        java.util.LinkedHashSet<String> out = new java.util.LinkedHashSet<String>();
        collectAttributeValues(currentDocument, ATTR_EVENT_ID, out);
        collectAttributeValues(currentDocument, ATTR_TRIGGER_EVENT_ID, out);
        collectAttributeValues(currentDocument, "data-sketsa-physics-event-id", out);
        return new java.util.ArrayList<String>(out);
    }

    private static void collectAttributeValues(Document doc, String attr, java.util.Set<String> out) {
    ...
    private void applyAudio() {
        ...
        beginUndoTransaction(undo, "Apply Audio");
        try {
            
            e.setAttribute(ATTR_SRC,src);
            e.setAttribute(ATTR_AUTOPLAY,Boolean.toString(autoplay.isSelected()));
            e.setAttribute(ATTR_LOOP,Boolean.toString(loop.isSelected()));
            e.setAttribute(ATTR_VOLUME,String.valueOf(((Number)volume.getValue()).doubleValue()));
            e.setAttribute(ATTR_PLAYBACK_RATE,String.valueOf(((Number)playbackRate.getValue()).doubleValue()));
            e.setAttribute(ATTR_START_OFFSET,String.valueOf(((Number)startOffset.getValue()).doubleValue()));
            e.setAttribute(ATTR_EVENT_ID,eventId.getText().trim());
            e.setAttribute(ATTR_MUTE,Boolean.toString(mute.isSelected()));
            e.setAttribute(ATTR_PAN,String.valueOf(((Number)pan.getValue()).doubleValue()));
            e.setAttribute(ATTR_PAN_MODE,autoPan.isSelected()?"svg-x":"manual");
            e.setAttribute(ATTR_TRIGGER_SOURCE,triggerSource.getText().trim());
            e.setAttribute(ATTR_TRIGGER_TYPE,triggerType.getText().trim());
            e.setAttribute(ATTR_TRIGGER_EVENT_ID,triggerEventId.getText().trim());
            e.setAttribute(ATTR_TRIGGER_ACTION,triggerAction.getText().trim());
            String busId=normalizeBusId(bus.getText());
            e.setAttribute(ATTR_BUS,busId);
            Element root=currentDocument==null?null:currentDocument.getDocumentElement();
            if(root!=null){
    ...
    private void removeAudio() {
        ...
        Element e=(Element)selected;
        DOMUndoManager undo=canvas.getUndoManager();
        beginUndoTransaction(undo, "Remove Audio");
        try {
            
            for (String a : new String[]{ATTR_SRC,ATTR_AUTOPLAY,ATTR_LOOP,ATTR_VOLUME,ATTR_PLAYBACK_RATE,ATTR_START_OFFSET,ATTR_EVENT_ID,ATTR_MUTE,ATTR_PAN,ATTR_PAN_MODE,ATTR_BUS,ATTR_TRIGGER_SOURCE,ATTR_TRIGGER_TYPE,ATTR_TRIGGER_EVENT_ID,ATTR_TRIGGER_ACTION}) e.removeAttribute(a);
            selected=e instanceof SVGElement ? (SVGElement)e : null;
            currentDocument=e.getOwnerDocument();
            undo.end();
