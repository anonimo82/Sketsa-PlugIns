...
final class PatternsPanel extends JPanel {
    ...
    private void applyPaint(String property) {
        ...
        if (TYPE_CUSTOM.equals(typeField.getSelectedItem())) {
            ...
            if (existing == null) {
                status.setText("No existing custom pattern #" + baseId + " found.");
                return;
            }
            draft = (Element) existing.cloneNode(true);
        } else {
            /*
             * Always rebuild from CURRENT controls at Apply time. This means
             * Apply works even if the user changes a field after Prepare.
             */
            String colorA = normalizeColorText(colorAField.getText(), "#ffffff");
            String colorB = normalizeColorText(colorBField.getText(), "#808080");
        ...
        PaintState before = PaintState.capture(cachedElement);

        Element defs = ensureDefs(document);
        defs.appendChild(appliedPattern);

        if (cachedElement instanceof SVGStylable) {
            DOMUtilities.updateProperty(
                    (SVGStylable) cachedElement, null, property,
                    "url(#" + uniqueId + ")");
        } else {
            cachedElement.setAttribute(property, "url(#" + uniqueId + ")");
        }

        PaintState after = PaintState.capture(cachedElement);

        canvas.getUndoManager().addEdit(
                new PrivatePatternApplyEdit(
                        appliedPattern,
                        cachedElement,
                        before,
                        after,
                        canvas,
                        "fill".equals(property)
                                ? "Apply Pattern Fill"
                                : "Apply Pattern Stroke"));
    ...
    private void removePatternPaint(String property) {
        ...
        canvas.getUndoManager().addEdit(
                new PaintEdit(cachedElement, before, after, canvas,
                        "fill".equals(property) ? "Remove Pattern Fill" : "Remove Pattern Stroke"));
