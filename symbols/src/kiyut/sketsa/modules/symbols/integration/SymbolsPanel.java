...
final class SymbolsPanel extends JPanel {
    ...
    private void createOrUpdateSymbol() {
        ...
        /*
         * Store a clone, never the live selected element itself.
         * Remove only the clone's top-level id to avoid an immediate duplicate
         * id between the original object and the definition.
         */
        ...
        if (existing == null) {
            ...
            existing.getParentNode().replaceChild(symbol, existing);
        }

        Element newSnapshot = (Element)symbol.cloneNode(true);

        canvas.getUndoManager().addEdit(
                new SymbolDefinitionEdit(
                        document, id,
        ...
        /*
         * Batik may keep an already-resolved <use> rendering cached after
         * replacing the referenced <symbol>. Reapply the same href values to
         * matching instances so Batik rebuilds their bridge/rendering.
         */
    ...
    private void insertUse() {
        ...
        Document document = cachedElement.getOwnerDocument();
        Element symbol = findSymbol(document, id);
        ...
        if (parent == null) {
            status.setText("Cannot insert instance here.");
            return;
        }

        Node next = cachedElement.getNextSibling();
        if (next == null) parent.appendChild(use);
        else parent.insertBefore(use, next);

        canvas.getUndoManager().addEdit(
                new InsertUseEdit(parent, use, next, canvas));
    ...
    private void updateUse() {
        if (canvas == null || cachedElement == null
                || !"use".equals(localName(cachedElement))) return;
        ...
        UseState before = UseState.capture(cachedElement);
        ...
        UseState after = UseState.capture(cachedElement);

        canvas.getUndoManager().addEdit(
                new UseStateEdit(cachedElement, before, after, canvas));
    ...
    private void detachUse() {
        ...
        if (symbol == null) {
            status.setText("Referenced symbol cannot be found.");
            return;
        }

        Node parent = cachedElement.getParentNode();
        if (parent == null) {
            status.setText("Cannot detach this instance.");
            return;
        }
        ...
        for (Node n = symbol.getFirstChild(); n != null; n = n.getNextSibling()) {
            group.appendChild(n.cloneNode(true));
        }

        Element useSnapshot = (Element)cachedElement.cloneNode(true);
        Node next = cachedElement.getNextSibling();
        ...
        canvas.getUndoManager().addEdit(
                new DetachUseEdit(
                        parent, useSnapshot, group, next, canvas));
