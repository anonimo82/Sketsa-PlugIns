...
final class InputPanel extends JPanel {
    ...
    private void applyInput(){ if(loading||canvas==null||!(selected instanceof Element))return; String a=action.getText().trim();String k=normalizeKeys(keys.getText());String ct=controlType.getText().trim().toLowerCase();boolean hasGamepad=!gamepadButton.getText().trim().isEmpty()||!gamepadAxis.getText().trim().isEmpty();boolean hasDirectional=!controlLeft.getText().trim().isEmpty()||!controlRight.getText().trim().isEmpty()||!controlUp.getText().trim().isEmpty()||!controlDown.getText().trim().isEmpty();boolean hasControl=!ct.isEmpty();if(!ct.isEmpty()&&!ct.equals("button")&&!ct.equals("dpad")&&!ct.equals("stick")){status.setText("On-screen control must be blank, button, dpad or stick.");return;}if(a.isEmpty()&&!(hasControl&&hasDirectional)){status.setText("Set Action, or directional actions for dpad/stick.");return;}if(!hasControl&&k.isEmpty()&&!hasGamepad){status.setText("Add a keyboard, gamepad or on-screen binding.");return;}if("button".equals(ct)&&a.isEmpty()){status.setText("Virtual button requires Action.");return;}if(("dpad".equals(ct)||"stick".equals(ct))&&!hasDirectional){status.setText("D-pad/stick requires at least one directional action.");return;}Element e=(Element)selected;DOMUndoManager undo=canvas.getUndoManager();undo.start("Apply Input");try{Element r=(Element)e.cloneNode(true);setOrRemove(r,ATTR_ACTION,a);setOrRemove(r,ATTR_KEYS,k);r.setAttribute(ATTR_PREVENT,Boolean.toString(preventDefault.isSelected()));setOrRemove(r,ATTR_GP_BUTTON,gamepadButton.getText().trim());setOrRemove(r,ATTR_GP_AXIS,gamepadAxis.getText().trim());setOrRemove(r,ATTR_GP_DIRECTION,gamepadDirection.getText().trim());setOrRemove(r,ATTR_GP_DEADZONE,gamepadDeadzone.getText().trim());setOrRemove(r,ATTR_GP_INDEX,gamepadIndex.getText().trim());setOrRemove(r,ATTR_CONTROL,ct);setOrRemove(r,ATTR_CONTROL_LEFT,controlLeft.getText().trim());setOrRemove(r,ATTR_CONTROL_RIGHT,controlRight.getText().trim());setOrRemove(r,ATTR_CONTROL_UP,controlUp.getText().trim());setOrRemove(r,ATTR_CONTROL_DOWN,controlDown.getText().trim());setOrRemove(r,ATTR_CONTROL_DEADZONE,controlDeadzone.getText().trim());setOrRemove(r,ATTR_TARGET_SOURCE,targetSource.getText().trim().toLowerCase());setOrRemove(r,ATTR_TARGET_ACTION,targetAction.getText().trim());setOrRemove(r,ATTR_TARGET_ID,targetId.getText().trim());setOrRemove(r,ATTR_TARGET_EVENT,targetEvent.getText().trim().toLowerCase());setOrRemove(r,ATTR_TARGET_X,targetX.getText().trim());setOrRemove(r,ATTR_TARGET_Y,targetY.getText().trim());if(e.getParentNode()==null)throw new IllegalStateException("Selected object has no parent.");e.getParentNode().replaceChild(r,e);selected=r instanceof SVGElement?(SVGElement)r:null;currentDocument=r.getOwnerDocument();undo.end();status.setText("Input mapping applied.");}catch(RuntimeException ex){undo.cancel();status.setText("Could not apply input: "+ex.getMessage());}updateEnabledState(); }
    private void removeInput(){ if(canvas==null||!(selected instanceof Element))return;Element e=(Element)selected;DOMUndoManager undo=canvas.getUndoManager();undo.start("Remove Input");try{Element r=(Element)e.cloneNode(true);r.removeAttribute(ATTR_ACTION);r.removeAttribute(ATTR_KEYS);r.removeAttribute(ATTR_PREVENT);r.removeAttribute(ATTR_GP_BUTTON);r.removeAttribute(ATTR_GP_AXIS);r.removeAttribute(ATTR_GP_DIRECTION);r.removeAttribute(ATTR_GP_DEADZONE);r.removeAttribute(ATTR_GP_INDEX);r.removeAttribute(ATTR_CONTROL);r.removeAttribute(ATTR_CONTROL_LEFT);r.removeAttribute(ATTR_CONTROL_RIGHT);r.removeAttribute(ATTR_CONTROL_UP);r.removeAttribute(ATTR_CONTROL_DOWN);r.removeAttribute(ATTR_CONTROL_DEADZONE);r.removeAttribute(ATTR_TARGET_SOURCE);r.removeAttribute(ATTR_TARGET_ACTION);r.removeAttribute(ATTR_TARGET_ID);r.removeAttribute(ATTR_TARGET_EVENT);r.removeAttribute(ATTR_TARGET_X);r.removeAttribute(ATTR_TARGET_Y);if(e.getParentNode()==null)throw new IllegalStateException("Selected object has no parent.");e.getParentNode().replaceChild(r,e);selected=r instanceof SVGElement?(SVGElement)r:null;currentDocument=r.getOwnerDocument();undo.end();status.setText("Input mapping removed.");}catch(RuntimeException ex){undo.cancel();status.setText("Could not remove input: "+ex.getMessage());}updateEnabledState(); }
