...
final class RuntimeHtmlExporter {
    ...
    static void export(Document document, File htmlFile, boolean includeCompanionRuntimes) throws Exception {
        ...
        boolean physics = hasPhysics(copy);
        boolean audio = includeCompanionRuntimes && AudioCompanionExporter.hasAudio(copy);
        boolean input = includeCompanionRuntimes && InputCompanionExporter.hasInput(copy);
        ...
        if (input) InputCompanionExporter.prepareCompanionAssets(assets);
        if (audio) AudioCompanionExporter.prepareCompanionAssets(copy, assets, assetPath);
        if (physics) {
        ...
        String html = combinedHtml(serialize(copy), assetPath, physics, audio, input);
    ...
    private static void cleanupGeneratedAssets(File assets) throws IOException {
        ...
        Files.deleteIfExists(new File(assets, "sketsa-audio-runtime.js").toPath());
        Files.deleteIfExists(new File(assets, "sketsa-audio-asset-data.js").toPath());
    ...
    private static void deleteRecursively(File file) throws IOException {
        ...
        if(!file.delete() && file.exists()) throw new IOException("Could not clean generated runtime asset: "+file);
    }
    ...
    private static String combinedHtml(String svg, String assetPath, boolean physics, boolean audio, boolean input) {
        StringBuilder h = new StringBuilder();
        h.append("<!doctype html>\n<html><head><meta charset=\"utf-8\">\n");
        h.append("<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">\n");
        if (physics) h.append("<meta name=\"sketsa-physics-runtime\" content=\"0.9.6; contract 1.0; matter-js ").append(MATTER_VERSION).append("\">\n");
        if (audio) h.append(AudioCompanionExporter.companionHeadHtml());
        if (input) h.append(InputCompanionExporter.companionHeadHtml());
        ...
        h.append("</head><body>\n");
        if (input) h.append(InputCompanionExporter.companionControlsHtml());
        if (audio) h.append(AudioCompanionExporter.companionControlsHtml());
        if (physics) {
            h.append("<div id=\"sketsa-physics-controls\"><button id=\"sketsa-physics-pause\" type=\"button\">Pause</button><button id=\"sketsa-physics-reset\" type=\"button\">Reset</button><button id=\"sketsa-physics-sleep\" type=\"button\">Sleep dynamics</button><button id=\"sketsa-physics-wake\" type=\"button\">Wake dynamics</button><span id=\"sketsa-physics-status\">Running</span></div>\n");
            h.append("<div id=\"sketsa-physics-events\"><strong>Physics events</strong><span id=\"sketsa-physics-event-summary\">start 0 · active 0 · end 0</span><ol id=\"sketsa-physics-event-log\"></ol></div>\n");
        }
        h.append(svg).append("\n");
        // Companion consumers load before Physics so their neutral-event listeners are ready before simulation begins.
        if (input) h.append(InputCompanionExporter.companionScriptsHtml(assetPath));
        if (audio) h.append(AudioCompanionExporter.companionScriptsHtml(assetPath));
        if (physics) {
            h.append("<script src=\"").append(assetPath).append("/matter.min.js\"></script>\n");
            h.append("<script src=\"").append(assetPath).append("/sketsa-physics-runtime.js\"></script>\n");
        }
        h.append("</body></html>\n");
        return h.toString();
    }
    ...
    private static void downloadMatter(File cache) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "Sketsa-Physics/0.9.6");
    ...
    private static String runtimeScript() {
        ...
        s.append("const CONTRACT_VERSION='1.0', RUNTIME_VERSION='0.9.6';\n");
