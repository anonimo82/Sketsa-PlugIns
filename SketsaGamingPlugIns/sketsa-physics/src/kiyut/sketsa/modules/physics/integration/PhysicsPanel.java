...
final class PhysicsPanel extends JPanel {
    ...
    private void applyPhysics() {
        ...
        undo.start("Apply Physics");
        try {
            Element replacement = (Element) e.cloneNode(true);
            replacement.setAttribute(ATTR_BODY, String.valueOf(bodyType.getSelectedItem()));
            replacement.setAttribute(ATTR_SHAPE, String.valueOf(shape.getSelectedItem()));
            replacement.setAttribute(ATTR_DENSITY, number(density));
            replacement.setAttribute(ATTR_MASS, number(mass));
            replacement.setAttribute(ATTR_FRICTION, number(friction));
            replacement.setAttribute(ATTR_FRICTION_STATIC, number(frictionStatic));
            replacement.setAttribute(ATTR_FRICTION_AIR, number(frictionAir));
            replacement.setAttribute(ATTR_RESTITUTION, number(restitution));
            replacement.setAttribute(ATTR_ANGLE, number(angle));
            replacement.setAttribute(ATTR_VELOCITY_X, number(velocityX));
            replacement.setAttribute(ATTR_VELOCITY_Y, number(velocityY));
            replacement.setAttribute(ATTR_ANGULAR_VELOCITY, number(angularVelocity));
            replacement.setAttribute(ATTR_SENSOR, Boolean.toString(sensor.isSelected()));
            replacement.setAttribute(ATTR_FORCE_X, number(forceX));
            replacement.setAttribute(ATTR_FORCE_Y, number(forceY));
            replacement.setAttribute(ATTR_IMPULSE_X, number(impulseX));
            replacement.setAttribute(ATTR_IMPULSE_Y, number(impulseY));
            replacement.setAttribute(ATTR_TORQUE, number(torque));
            replacement.setAttribute(ATTR_SLEEPING, Boolean.toString(sleeping.isSelected()));
            replacement.setAttribute(ATTR_CONSTRAINT_TYPE, String.valueOf(constraintType.getSelectedItem()));
            replacement.setAttribute(ATTR_CONSTRAINT_TARGET, constraintTarget.getText().trim());
            replacement.setAttribute(ATTR_CONSTRAINT_POINT_A_X, number(constraintPointAX));
            replacement.setAttribute(ATTR_CONSTRAINT_POINT_A_Y, number(constraintPointAY));
            replacement.setAttribute(ATTR_CONSTRAINT_POINT_B_X, number(constraintPointBX));
            replacement.setAttribute(ATTR_CONSTRAINT_POINT_B_Y, number(constraintPointBY));
            replacement.setAttribute(ATTR_CONSTRAINT_LENGTH, number(constraintLength));
            replacement.setAttribute(ATTR_CONSTRAINT_STIFFNESS, number(constraintStiffness));
            replacement.setAttribute(ATTR_CONSTRAINT_DAMPING, number(constraintDamping));
            replacement.setAttribute(ATTR_COLLISION_CATEGORY, integer(collisionCategory));
            replacement.setAttribute(ATTR_COLLISION_MASK, integer(collisionMask));
            replacement.setAttribute(ATTR_COLLISION_GROUP, integer(collisionGroup));
            replacement.setAttribute(ATTR_EVENT_ID, eventId.getText().trim());
            replacement.setAttribute(ATTR_EVENT_START, Boolean.toString(eventStart.isSelected()));
            replacement.setAttribute(ATTR_EVENT_ACTIVE, Boolean.toString(eventActive.isSelected()));
            replacement.setAttribute(ATTR_EVENT_END, Boolean.toString(eventEnd.isSelected()));
    ...
    private void removePhysics() {
        ...
        try {
            ...
            for (String attr : attrs) replacement.removeAttribute(attr);
            if (e.getParentNode() == null) throw new IllegalStateException("Selected object has no parent.");
            e.getParentNode().replaceChild(replacement, e);
            selected = replacement instanceof SVGElement ? (SVGElement) replacement : null;
            currentDocument = replacement.getOwnerDocument();
    ...
    private void applyWorld() {
        ...
        Element root = currentDocument.getDocumentElement();
        DOMUndoManager undo = canvas.getUndoManager();
        undo.start("Apply Physics World");
        try {
            Element replacement = (Element) root.cloneNode(true);
            replacement.setAttribute(ATTR_GRAVITY_X, number(gravityX));
            replacement.setAttribute(ATTR_GRAVITY_Y, number(gravityY));
            replacement.setAttribute(ATTR_GRAVITY_SCALE, number(gravityScale));
            replacement.setAttribute(ATTR_TIME_SCALE, number(timeScale));
            replacement.setAttribute(ATTR_ENABLE_SLEEPING, Boolean.toString(enableSleeping.isSelected()));
            currentDocument.replaceChild(replacement, root);
            currentDocument = replacement.getOwnerDocument();
            selected = null;
            undo.end();
