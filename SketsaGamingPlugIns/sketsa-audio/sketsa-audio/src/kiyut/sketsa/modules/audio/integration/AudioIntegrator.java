...
public final class AudioIntegrator {
    private static final String PANEL_NAME = "SketsaAudioWebAudio030Panel";
    private AudioIntegrator() {}

    public static boolean install() {
        TopComponent tc = WindowManager.getDefault().findTopComponent("PropertiesTopComponent");
        if (!(tc instanceof PropertiesTopComponent)) return false;
        if (findByName(tc, PANEL_NAME) != null) return true;
        ...
        AudioPanel audio = new AudioPanel();
        audio.setName(PANEL_NAME);
        holder.add(nativeHolder, BorderLayout.CENTER);
        holder.add(audio, BorderLayout.SOUTH);
